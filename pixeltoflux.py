...
def get_pixelfluxes(filepath,outputfolder='',starname=''):
  # Read a pixel file, and return the flux per pixel over time
  pixfile = pyfits.open(filepath,memmap=True)  # open a FITS file
  #print f.info()
  ...
  print ("Printing in get_pixelfluxes", keplerid)
...
def find_aperture(dates,fluxes,plot=True,starname='',outputfolder='',kepmag='na',cutoff_limit=2.):
  ...
  flux = np.nansum(fluxes,axis=0)/len(fluxes)
...
def gotoflux(filepath,outputpath='',campaign=2,cutoff_limit=2.):
  '''
  ...
  dates,fluxes,kepmag,Xabs,Yabs = get_pixelfluxes(filepath,outputfolder=outputfolder,starname=starname)
